...
class T_win:
    def __init__(self, t_i, y, t, interval,
                 n_start,n_stop,n_step,n_break_criteria,amp_limit_ratio,freq_limit,
    ...
    # def prony_n(self, n_start,n_stop,n_step,n_break_criteria,amp_limit_ratio,freq_limit):
        ...
        for n in range(n_start,n_stop,n_step):
            ...
            # hard limit
            # amp_limit = np.amax(prony_shape[:,2]) *amp_limit_ratio  # relative to highest amplitude
            ...
            # prony_shape_hl = np.delete(prony_shape_hl,np.where(prony_shape_hl[:,0]>dr_limit),axis=0)
    ...
    # def t_win_plot(self, scrollable_frame):
        ...
        self.ax.set_title(f'Time window: {t_i} - {t_i+interval}', fontsize=16)
...
def run_cluster(eps, min_samples, ax2, canvas, prony_gui, checkbox_list, var_chooseall, figure_p, root):
    ...
    for win_t_i in checkbox_list:
        # print(globals()[win_t_i].var1.get())
    ...
    ax2.set_xlabel('Damping', fontsize=12)
    ax2.set_ylabel('Freq [Hz]', fontsize=12)
    for xy in modes:
    ...
    tk.Button(prony_gui, text='compare with other data', command=lambda: compare(
        title, modes, root)).grid(row=4, column=3, sticky='w')
...
def run_prony(root, start_time, interval, step, stop_time,
    ...
    time_win_frame.grid(row=0, column=0, columnspan=4, padx=10, pady=10, sticky='nw')
    ...
    scrollable_frame = tk.LabelFrame(canvas_twin, text='scrollable frame')
    ...
    figure_p = plt.Figure(figsize=(12,5))
